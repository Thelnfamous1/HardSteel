...
public class BlastFurnaceContainer extends Container {
    ...
    public BlastFurnaceContainer(int id, PlayerInventory playerInventoryIn) {
        this(id, playerInventoryIn, new IntArray(5), new Inventory(4), null);
    }

    public BlastFurnaceContainer(int id, PlayerInventory playerInventoryIn, IIntArray machineData, IInventory machineInv, BlockPos tePos) {
        super(BlastFurnaceRegistryEvents.BLAST_FURNACE_CONTAINER, id);
        this.tePos = tePos;
        ...
        addSlot(new BlastFurnaceResultSlot(playerInventoryIn.player, machineInv, 3, 116, 35));
    ...
    public ItemStack transferStackInSlot(PlayerEntity playerIn, int index) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index == 0 || index == 1 || index == 2 || index == 3) {
                ...
                } else if(stack.getItem() == ItemList.limestone) {
    ...
    private boolean hasRecipe(ItemStack stack) {
        ...
        for(ItemStack s : this.getInventory()) {
            if(s.getItem() == ItemList.limestone) {
                flag = this.world.getRecipeManager().getRecipe((IRecipeType<HotBlastingRecipe>)IHardSteelRecipeType.HOT_BLASTING, new Inventory(ItemStack.EMPTY, new ItemStack(ItemList.limestone), stack), this.world).isPresent();
            }
        }
        return flag || this.world.getRecipeManager().getRecipe((IRecipeType<HotBlastingRecipe>)IHardSteelRecipeType.HOT_BLASTING, new Inventory(ItemStack.EMPTY, ItemStack.EMPTY, stack), this.world).isPresent();
    }
