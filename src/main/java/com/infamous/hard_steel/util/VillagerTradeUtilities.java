...
public class VillagerTradeUtilities {
    ...
    public static class EnchantedItemForEmeraldsTrade implements VillagerTrades.ITrade {
        ...
        public EnchantedItemForEmeraldsTrade(Item item, int stackSize, int maxUses, int givenExp) {
            this(item, stackSize, maxUses, givenExp, 0.05F);
        }

        public EnchantedItemForEmeraldsTrade(Item item, int stackSize, int maxUses, int givenExp, float priceMultiplier) {
        ...
        public MerchantOffer getOffer(Entity p_221182_1_, Random random) {
            ...
            ItemStack enchantedItem = EnchantmentHelper.addRandomEnchantment(random, new ItemStack(this.itemStack.getItem()), level, false);
            //p_77504_3_ is allowTreasure
            int priceBasedOnRarity = Math.min(this.stackSize + level, 64);
            ItemStack emeraldPrice = new ItemStack(Items.EMERALD, priceBasedOnRarity);
            return new MerchantOffer(emeraldPrice, enchantedItem, this.maxUses, this.givenExp, this.priceMultiplier);
        }
    }
