...
public class SmithingTableTileEntityRenderer extends TileEntityRenderer<SmithingTableTileEntity> {
    ...
    @Override
    public void render(SmithingTableTileEntity tileEntity, double x, double y, double z, float partialTicks, int destroyStage) {
        if (!tileEntity.handler.getContents().isEmpty() && this.rendererDispatcher.renderInfo != null && tileEntity.getDistanceSq(this.rendererDispatcher.renderInfo.getProjectedView().x, this.rendererDispatcher.renderInfo.getProjectedView().y, this.rendererDispatcher.renderInfo.getProjectedView().z) < 128d) {
            ...
            Direction dir = tileEntity.getBlockState().get(BlockSmithingTable.FACING);
