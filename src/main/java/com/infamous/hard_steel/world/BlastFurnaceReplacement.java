...
public class BlastFurnaceReplacement extends Feature<NoFeatureConfig> {

    public BlastFurnaceReplacement(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn) {
            ...
            for (int x = 0; x < 16; ++x) {
                for (int y = 0; y < worldIn.getMaxHeight(); ++y) {
                    for (int z = 0; z < 16; ++z) {
                        ...
                        if (worldIn.getBlockState(blockPos$mutableBlockPos).getBlock() == Blocks.BLAST_FURNACE) {
                            ...
                            //}
                            //else
                            worldIn.setBlockState(blockPos$mutableBlockPos, BlastFurnaceRegistryEvents.BLAST_FURNACE.getDefaultState(), 3);
                        }
                    }
                }
            }
